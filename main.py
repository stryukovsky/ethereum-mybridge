    ...
    while True:
        ...
        for event in events:
            ...
            if can_finalize:
                ...
                tx_hash = send_tx(bridge_finalizer_account, contract_finalizer, bridge_finalize_data, web3_finalizer)
                print(f"Sent to finalizer-chain tx {prettify_tx_hash(tx_hash)}")
                receipt = web3_finalizer.eth.wait_for_transaction_receipt(tx_hash)
                if receipt.status == 1:
                    ...
                    send_tx(bridge_initializer_account, contract_initializer, bridge_complete_data, web3_initializer)
            else:
